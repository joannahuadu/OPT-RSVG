...
from utils.box_utils import bbox_iou, xywh2xyxy, xyxy2xywh, generalized_box_iou,eiou_box_iou
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred_label, target):
        target = torch.tensor([torch.tensor(i) for i in target])
        target = target.to(pred_label.device)
        ce_loss = F.cross_entropy(pred_label, target, reduction='none')
...
def trans_vg_loss(batch_pred, batch_target):
    ...
    # 16 * 1
    # 16 * 18
    #loss_cls = focal_loss(pred_label,label)
    ...
    # loss_giou = torch.zeros((batch_size,4))
    losses = {}
