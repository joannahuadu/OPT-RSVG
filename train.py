...
from datasets.data_loader_new import *
...
def get_args_parser():
    ...
    parser.add_argument('--detr_model', default='../pretrained/detr-r50-e632da11.pth', type=str, help='detr model')
...
def main(args):
    os.environ['MASTER_PORT'] = '8881'  # 端口号随意指定
    ...
    a = [n for n, p in model.named_parameters() if not p.requires_grad]
    for i in a:
        print(i)
    ...
    m_head = [p for n, p in model_without_ddp.named_parameters() if ("mhead" in n and p.requires_grad)]
    rest_param = [p for n, p in model_without_ddp.named_parameters() if (("mhead" not in n) and ("vhead" not in n)and("visumodel" not in n) and ("textmodel" not in n) and p.requires_grad)]
    param_list = [{"params": rest_param},
                   {"params": visu_cnn_param, "lr": args.lr_visu_cnn},
                   {"params": visu_tra_param, "lr": args.lr_visu_tra},
                  {"params": m_head, "lr": args.lr_visu_tra},
    ...
    if args.output_dir and utils.is_main_process():  # get_rank() == 0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_stats = {k: v for k, v in val_stats.items()}
        ...
        if args.output_dir:
            ...
            if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % 10 == 0 and (epoch+1) >= 90:
